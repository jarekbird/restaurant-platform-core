# TASK-003: Add a minimal sanity test for the app root
...
Add a minimal sanity test for the app root. This task involves: Add a test file `test/app-root.spec.tsx` that renders the default `app/page.tsx` and asserts a known text is present.; Run `npm test` and ensure it passes. This task is part of Tooling & Baseline App Health and contributes to building the restaurant platform core application.

**Reference Implementation**: `plan/phase-1/master-plan.md`

## Current State

- Previous tasks: [T-002]
...
**Important**: This task depends on: [T-002]
...
- [ ] Understand dependencies and prerequisites: [T-002]
...
- [ ] Step 1: Add a test file `test/app-root.spec.tsx` that renders the default `app/page.tsx` and asserts a known text is present.
- [ ] Step 2: Run `npm test` and ensure it passes.
...
- **Execution Timing**: Execute after completing dependencies: [T-002]
- **Dependencies**: [T-002]
...
- Previous: TASK-002 (T-002)
- Next: TASK-004 (T-004)
- Dependencies:
  - T-002: Configure testing environment (Vitest + React Testing Library)
