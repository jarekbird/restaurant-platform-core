# TASK-014: Populate `config.json` for `so-delicious`
...
Populate `config.json` for `so-delicious`. This task involves: Fill `data/restaurants/so-delicious/config.json` with realistic example data matching `restaurantConfigSchema`.; Include `theme` set to an existing or planned theme key (e.g., `"sushi-dark"`). This task is part of Phase 1: Domain Modeling and contributes to building the restaurant platform core application.

**Reference Implementation**: `plan/phase-1/master-plan.md`

## Current State

- Previous tasks: [T-024]
...
**Important**: This task depends on: [T-024]
...
- [ ] Understand dependencies and prerequisites: [T-024]
...
- [ ] Step 1: Fill `data/restaurants/so-delicious/config.json` with realistic example data matching `restaurantConfigSchema`.
- [ ] Step 2: Include `theme` set to an existing or planned theme key (e.g., `"sushi-dark"`).
...
- **Execution Timing**: Execute after completing dependencies: [T-024]
- **Dependencies**: [T-024]
