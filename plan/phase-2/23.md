# TASK-2-2-6: Implement `buildSystemPrompt` helper
...
- [ ] Step 1: Add a `buildSystemPrompt` function in `lib/ai/chatService.ts` (or a nearby helper).
- [ ] Step 2: Accept `menu` and `cart` and encode them into a concise system prompt, including role and capabilities.
