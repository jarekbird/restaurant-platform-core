# TASK-2-2-18: Integrate `ChatAssistant` into preview layout
...
- [ ] Step 1: Import `ChatAssistant` into `app/preview/[slug]/page.tsx` or a layout wrapper.
- [ ] Step 2: Pass menu data (or an identifier) and ensure `ChatAssistant` is within `CartProvider` so it can use `useCartContext`.
- [ ] Step 3: Verify cart changes initiated from chat are reflected in the UI cart drawer and badge.
