# TASK-2-2-4: Integrate `ChatMessage` and `ChatInput` into `ChatAssistant`
...
- [ ] Step 1: Inside `ChatAssistant`, manage local chat history state (array of messages).
- [ ] Step 2: Render the history using `ChatMessage` and the input using `ChatInput`.
- [ ] Step 3: Wire `onSend` to append a user message and trigger an AI call (to be implemented).
