# TASK-2-2-13: Wire `actionParser` results to cart operations
...
- [ ] Step 1: After receiving an AI response from `/api/chat`, run it through `parseChatAction`.
- [ ] Step 2: Based on the parsed `ChatAction`, call `addItem`, `removeItem`, or `updateQuantity` from `useCartContext`.
- [ ] Step 3: Append system/assistant confirmation messages reflecting what changed in the cart.
