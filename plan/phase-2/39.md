# TASK-2-3-4: Harden error handling for AI and cart operations
...
- [ ] Step 1: Add robust try/catch around calls to `/api/chat` and `sendChatMessage`.
- [ ] Step 2: Implement fallbacks when AI is unavailable (e.g., friendly error message in chat).
- [ ] Step 3: Validate all parsed actions (e.g., verify `itemId` exists in the menu before mutating cart).
